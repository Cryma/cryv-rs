...
macro_rules! native {
    ($type:ty, $hash:expr, $parameters:expr) => {{
        let native = crate::native_handling::map_native($hash).unwrap();
        let native_handler = crate::native_handling::get_native_handler(native).unwrap();

        crate::native_handling::invoke_init(native);
        $parameters;
        crate::native_handling::invoke_call::<$type>(native_handler)
    }};
}
...
macro_rules! native_parameters {
    ...
    ($parameter:expr) => {
        {
            crate::native_handling::invoke_push($parameter);
        }
    };
...
fn handler(hash: u64) -> Option<*mut c_void> {
    ...
    let registration_table = unsafe { crate::POINTERS.registration_table };
...
pub(crate) fn invoke_init(hash: u64) {
    unsafe {
...
pub(crate) fn invoke_push<T>(argument: T) {
    let mut value: u64 = 0;
...
pub(crate) fn invoke_call<R: Copy>(native_handler: *mut c_void) -> R {
